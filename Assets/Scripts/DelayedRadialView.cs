...
public class DelayedRadialView : Solver
{
    [SerializeField]
    ...
    private void GetDesiredOrientation(ref Vector3 desiredPos)
    {
        ...
        if (currentAngle == currentAngleClamped && clampedDistance.Equals(elementDist))
        {
            DelayReset();
            return;
        }
    ...
    private bool IsDelayReached()
    {
        return Time.time - delayStartTime >= delayTime;
    }
}
